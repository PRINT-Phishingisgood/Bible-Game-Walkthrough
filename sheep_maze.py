...
def draw_win_screen(surface, font_big, font_med, font_small, t):
    ...
    lines = [
        ...
        (font_small, "Press ESC/ENTER to return and record score", (150, 150, 150)),
    ]
    y = SCREEN_H // 2 - 100
...
def run(skin_data=None):
    ...
    verse_surf = font_small.render('"Go after the lost sheep until he finds it." — Luke 15:4', True, (160, 130, 80))

    grid = generate_maze(COLS, ROWS)
    maze_surf = build_maze_surface(grid)

    path_cells = [(c, r) for r in range(ROWS) for c in range(COLS) if grid[r][c] == PATH]
    ...
    for i in range(min(NUM_SHEEP, len(sheep_cells))):
        sc = sheep_cells[i]
        sheep_list.append({"x": sc[0] * CELL + CELL // 2, "y": sc[1] * CELL + CELL // 2, "found": False})
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                # Return standard metrics upon completion escape commands
                if event.key == pygame.K_ESCAPE:
                    running = False
                if event.key in (pygame.K_RETURN, pygame.K_KP_ENTER, pygame.K_ESCAPE) and game_won:
                    # Score computation tied directly to total search duration efficiency
                    # Multiplies seconds by 15 instead of 120, giving players a generous ~5-minute window
                    calculated_score = max(500, 5000 - int(t * 15))
        ...
        if not game_won:
            ...
            def can_stand(x, y):
                ...
                for cx_, cy_ in corners:
                    col_ = int(cx_) // CELL
                    row_ = int(cy_) // CELL
                    if not (0 <= col_ < COLS and 0 <= row_ < ROWS) or grid[row_][col_] == WALL:
                        return False
                return True
        ...
        # Inject matching character skins to player avatar frames
        draw_player(screen, int(player_x), int(player_y), skin_data)

        fog = build_fog_surface(SCREEN_W, ROWS * CELL, int(player_x), int(player_y), VISION_RADIUS)
        screen.blit(fog, (0, 0))
