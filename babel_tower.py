...
def run(skin_data=None):
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE and game_state == "playing":
                    running = False
                if event.key in (pygame.K_RETURN, pygame.K_KP_ENTER, pygame.K_ESCAPE) and game_state == "won":
                    # Score derived primarily from speed, with heavy penalties for getting confused
                    score_calc = max(500, 12000 - int(t * 100) - (times_confused * 450))
                    return score_calc
        ...
        if game_state == "won":
            ...
            ret_msg = fnt_med.render("Press ESC/ENTER to return and record score", True, C_TEXT)
